          ...
          <AnimatePresence>
            {open && (
              <motion.div
                ...
                className="fixed inset-0 z-[300] flex min-h-[100dvh] flex-col items-center justify-center gap-4 overflow-y-auto bg-[#050505] px-6 py-24 md:hidden"
              >
                <button
                  type="button"
                  aria-label="Close menu"
                  onClick={() => setOpen(false)}
                  className="absolute right-5 top-4 flex h-10 w-10 items-center justify-center rounded-full border border-[color:var(--border)] text-chrome focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[color:var(--bronze)]"
                >
                  <span aria-hidden className="relative block h-5 w-5">
                    <span className="absolute left-0 top-1/2 h-px w-5 -translate-y-1/2 rotate-45 bg-chrome" />
                    <span className="absolute left-0 top-1/2 h-px w-5 -translate-y-1/2 -rotate-45 bg-chrome" />
                  </span>
                </button>
                {NAV.map((n, i) => (
                  ...
                  >
                    <Link
                      to={n.to}
                      onClick={() => setOpen(false)}
                      className="font-serif text-[clamp(2rem,10vw,3rem)] text-chrome transition-opacity hover:opacity-70 focus-visible:outline-none focus-visible:ring-1 focus-visible:ring-[color:var(--bronze)]"
                    >
                      {n.label}
                    </Link>
                  </motion.div>
                ...
                <motion.span
                  ...
                  className="tracking-luxe absolute bottom-8 text-[0.55rem] text-[color:var(--steel)]"
                >
                  Waliya · Atelier
                </motion.span>
              </motion.div>
